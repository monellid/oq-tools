...
SUPPORTED_PMFS = ['TRTPMF','MagDistEpsPMF']

def set_up_arg_parser():
	"""
	Set up command line parser.
	"""
	parser = argparse.ArgumentParser(description='Plot disaggregation results stored in a hdf5 file as produced by OpenQuake using GMT.')
	parser.add_argument('--config_file',help='path to OpenQuake configuration file used for generating disaggregation results.')
...
def plot_mag_dist_eps_pmf(pmf,mag,dist,eps,args):
	...
	annotation = '-B:Magnitude (Mw):%s/:Distance (km):%s/%s:Probability::.%s:WeSnZ' % (0.5,5.0,0.02,"Magnitude-Distance-Epsilon PMF")
		...
		for j in range(len(dist) - 2,-1,-1):
			...
			for k,v in enumerate(values):
				...
				if i == len(mag) - 2 and j==len(dist) - 2 and k == 0:
					call(["psxyz","hist.dat",region,projection,'-JZ8c',annotation,'-E200/30','-So0.5','-Wthinnest','-G'+color,'-K'],stdout=plot_file)
				else:
					call(["psxyz","hist.dat",region,projection,'-JZ8c','-E200/30','-So0.5b'+str(base_hight),'-Wthinnest','-G'+color,'-O','-K'],stdout=plot_file)
				
				call(["rm","hist.dat"])
				
	# plot legend
	hight,width = create_legend_file(eps,"legend.dat","Epsilon")
	legend_postion = "-Dx%sc/%sc/%sc/%sc/BL" % (float(args.xy_size) + width / 2, float(args.z_size) - hight / 2, width, hight)
...
def plot_trt_pmf(pmf,args):
	...
	call(["psxy","hist.dat",region,projection,annotation,"-Ggray","-Xc","-Yc","-Sb0.5","-K"],stdout=plot_file)
	call(["rm","hist.dat"])
